...
interface Body {
  ...
  title: string;
  description?: string;
  // Rental billing period. Only honored for `location`; monthly when omitted.
  per_month?: boolean;
  price_minor: number;
  bedrooms?: number;
  area_sqm?: number;
...
Deno.serve(makePost<Body>('/v1/properties/create', valid, async ({ sb, body, req }) => {
  ...
  // per_month is meaningful only for rentals. Daily rentals send per_month=false;
  // monthly (the historical default) when the client omits it. vente/terrain: false.
  const per_month = body.type === 'location' ? (body.per_month ?? true) : false;
