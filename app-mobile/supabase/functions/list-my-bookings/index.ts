// Tenant's own bookings, newest first, with the landlord's display name joined
// (second-query + Map convention). Powers /bookings.
...
interface Row {
  id: string; property_id: string; landlord_id: string; period: string;
...
Deno.serve(makePost<Body>('/v1/bookings/list-mine', valid, async ({ sb, body, req }) => {
  ...
  const { data, error } = await sb
    ...
    .limit(limit);
  if (error) { console.error('[list-my-bookings] query:', error); throwApi('INTERNAL_ERROR', 500, 'Erreur base de données'); }
  ...
  const bookings = rows.map((r) => ({
    ...
    counterpartyName: nameById.get(r.landlord_id) ?? null,
  }));
