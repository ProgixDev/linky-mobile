...
// Calls confirm_order_receipt RPC which verifies caller=buyer + status in
// (paid, delivered), then atomically splits the escrow: amount → seller wallet,
// fees → platform wallet. Order moves to status='released' with 'Réception
// confirmée' appended to events. Returns the updated order row.
