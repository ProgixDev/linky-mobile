# Pre-prod: apply addresses table + set_default_address RPC via Management API.
# Mirrors preprod-apply-shop-followers (db push is unusable per
...
$migration = '20260620_03_addresses'
...
Write-Host ''
Write-Host '=== Verify addresses table + set_default_address RPC exist ==='
$check = RunQuery "select (select count(*) from information_schema.tables where table_schema='public' and table_name='addresses') as table_count, (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='set_default_address') as rpc_count, (select count(*) from pg_indexes where schemaname='public' and tablename='addresses') as index_count;"
