# Phase I.3 apply — steps 1, 2, 4: generate LINKY_CRON_SECRET via CSPRNG,
# set 4 env vars on Supabase via Management API, write migration 02 with the
# secret literal substituted. Never echoes secret or PAT to stdout (length-only).
# Convention: multi-line PowerShell operations in Phase I+ live as scripts/*.ps1.

$ErrorActionPreference = 'Stop'

# --- Step 1: CSPRNG 32-byte base64 secret ----------------------------------
$bytes = New-Object byte[] 32
[System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($bytes)
$cron = [Convert]::ToBase64String($bytes)
$secretLen = $cron.Length

# --- Step 2: POST 4 env vars to Supabase Management API ---------------------
...
$mockUrl = "https://$ref.supabase.co/functions/v1/mock-lengopay"

$secrets = @(
  @{ name = 'LINKY_LENGOPAY_BASE_URL';    value = $mockUrl       }
  @{ name = 'LINKY_LENGOPAY_LICENSE_KEY'; value = 'mock-license' }
  @{ name = 'LINKY_LENGOPAY_WEBSITE_ID';  value = 'mock-website' }
  @{ name = 'LINKY_CRON_SECRET';          value = $cron          }
)
$body = ConvertTo-Json $secrets -Depth 3 -Compress
...
# --- Step 4: write migration 02 with $cron substituted ---------------------
# Single-quoted here-string preserves the plpgsql $func$ dollar-quote delimiter
# literally. Only the __CRON_SECRET__ placeholder gets replaced.
$template = @'
...
# --- Summary --------------------------------------------------------------
Write-Host ''
Write-Host '=== STEP 1-2-4 SUMMARY ==='
Write-Host "  secret generated:    yes (length: $secretLen)"
Write-Host '  env vars set:        4 (BASE_URL, LICENSE_KEY, WEBSITE_ID, CRON_SECRET)'
Write-Host "  migration 02 file:   $migrationPath"
Write-Host '  status:              READY for migration 01 apply'
