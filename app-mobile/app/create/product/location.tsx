// First-time-seller-only step: place the shop's exact pin before publishing
// their first product. Only reached when the caller has no shop yet (details.tsx
// skips straight to photos.tsx otherwise) — a shop auto-created without this
// step lands on the city centroid, which is why most boutiques weren't priced
// by distance (client note 2026-09-05). Mirrors create/property/location.tsx
// (same map, same required-pin gate) so the pattern stays in one place.
        ...
        <View style={{ aspectRatio: 1, borderRadius: 16, overflow: 'hidden', backgroundColor: colors.bgSunken, position: 'relative' }}>
          {Platform.OS === 'web' ? (
            ...
            >
              ...
              />
              {hasCoords && (
                <PointAnnotation id="shop-pin" coordinate={[lng!, lat!]}>
                  <View
                    style={{
                      width: 26,
                      height: 26,
